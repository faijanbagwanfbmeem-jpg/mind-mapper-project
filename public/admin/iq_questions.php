...
if (isset($_POST['add'])) {
    ...
    $stmt = $mysqli->prepare("
        INSERT INTO iq_questions (question, option_a, option_b, option_c, option_d, correct_option)
        VALUES (?, ?, ?, ?, ?, ?)
    ");
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $mysqli->query("DELETE FROM iq_questions WHERE id=$id");
    header("Location: iq_questions.php");
    exit;
}

$questions = $mysqli->query("SELECT * FROM iq_questions ORDER BY id DESC");
?>
...
<tr>
    ...
    <td>
        <a href="iq_questions.php?delete=<?= $q['id'] ?>" style="color:red;">Delete</a>
    </td>
</tr>
<?php endwhile; ?>
</table>
